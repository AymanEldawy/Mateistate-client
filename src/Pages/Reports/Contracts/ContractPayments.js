import { EyeIcon, PlusIcon, PrintIcon } from "Components/Icons";
...
import {
  getAccountCash,
  getVoucherLastNumber,
} from "Helpers/Lib/operations/global-read";
import { cost_center } from "Helpers/columns-structure";
import { METHODS, SELECT_LISTS } from "Helpers/constants";
    ...
    if (recordResponse?.table === CHQ_RECEIVED_NAME) {
      ...
      if (recordResponse?.method === METHODS.INSERT) {
        if (response?.success) {
          ...
          setValue(`voucher_grid.${index}`, grid?.at(0));
          setRecordResponse({});
        }
      } else {
      }
      ...
      <div>
        ...
        {watch("installment_grid")?.length ? (
          <TableFields
            ...
            allowViewEntry={(row) => {
              if (row?.id) {
                console.log("🚀 ~ ContractPayments ~ row:", row);
